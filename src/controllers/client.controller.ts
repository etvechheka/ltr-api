...
export const createClient = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const result: any = await getClientByEmail(email as string);
        if (result.length !== 0) {
...
export const getClient = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const result: any = await getClientById(id as string);
        if (result.length !== 0) {
            return res.status(200).json({
                status: true,
...
export const updateClient = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        const passwordHash = password !== '' ? await encryptPassword(password) : result[0].client_password;
        const newData = {
...
export const loginClient = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const result: any = await getClientByEmail(client_username);
        if (result.length !== 0) {
